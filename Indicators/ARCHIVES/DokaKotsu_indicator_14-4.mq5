//+------------------------------------------------------------------+
//|                              DokaKotsu_indicator_14.mq5            |
//|   バージョン : Ver14.0(_14)  修正日 : 2026-07-12                   |
//|                                                                  |
...
input bool   InpEntryModeBBKCOnly = true;  // ★2026-07-13追加(パターンB): true=実際のエントリー判定をBB×KC(regimeSqueeze)+再エントリーロックのみで行い、M15/Wave/ZigZag/ADX/長期MTF等の既存フィルターは判定に使わず参考記録(buf59)のみにする。false=従来通り全フィルターを使う(昨日までのロジック)。決済(EXIT)ロジックには一切影響しない。
...
double BufOldChainReason[];         // ★2026-07-13追加: パターンB(InpEntryModeBBKCOnly)運用時の参考列。
...
void LoadStateCacheAndOpenHandle()
{
   ...
   if(InpResetStateLog && FileIsExist(g_stateLogPath))
   {
      ...
      Print("[indicator_14] InpResetStateLog=true のため状態ログを削除しました: ", g_stateLogPath);
   }
   ...
   if(g_stateFileHandle == INVALID_HANDLE)
   {
      Print("[indicator_14] 状態ログを開けません err=", GetLastError(), " path=", g_stateLogPath);
      return;
   }
   ...
   Print("[indicator_14] 状態ログ読込完了: ", g_stateCacheCount, "件 path=", g_stateLogPath);
}
...
int OnInit()
{
   ...
   if(InpUseExternalSpikeForState)
   {
      ...
      if(hSpikeFilter == INVALID_HANDLE)
         Print("[indicator_14] ", InpSpikeFilterName, " のハンドル作成失敗。マーケットステイトのSPは常にfalse扱いになります。");
   }
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "DokaKotsu_indicator_14"); // 2026-07-12修正: 13→14へバージョンアップに伴いファイル名と一致させる
   ...
   for(int i=need; i<rates_total; i++)
   {
      ...
      if(pos==0 && !justExited)
      {
         ...
         else
         {
            ...
            // ★2026-07-13追加: パターンB(InpEntryModeBBKCOnly)運用。
            //   ここまでの`allow`は従来通りの全フィルター判定結果(昨日までのロジック)。
            //   実際の発注(BufBuy/BufSell,pos/segHadEntry/trendDirの更新)は、トグルONの間だけ
            //   BB×KC(regimeSqueeze)+再エントリーロックのみで行う。旧ロジックの判定は捨てず、
            //   BufOldChainReason(buf59)に「もし従来ロジックのままだったらどうだったか」を記録する。
            int oldChainReason = (int)BufReason[i];      // 0=旧ロジックなら許可していたはず/それ以外=旧ロジックのブロック理由コード
            ...
            bool allowFinal = InpEntryModeBBKCOnly ? allowBBKC : allow;
