//+------------------------------------------------------------------+
//|                              DokaKotsu_indicator_10.mq5            |
//|   バージョン : Ver10.0(_10)  修正日 : 2026-06-28 (波統合/長期先頭/Waveトリガー)|
//|                                                                  |
//|  ■■ 最重要・絶対ルール(EAと共通) ■■                            |
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "DokaKotsu_indicator_10"); // ファイル名と一致させ括弧表示を解消
   ...
   for(int i=need; i<rates_total; i++)
   {
      ...
      if(pos==0 && !justExited)
      {
         ...
         else
         {
            ...
            if(allow && InpUseLongFilter && longDir != d) { allow = false; BufReason[i]=22.0; } // 長期足不一致(グレー/逆)
