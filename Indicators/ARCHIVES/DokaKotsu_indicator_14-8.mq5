...
double BufReentryLock[];            // ★2026-07-14追加: 再エントリーロック状態(1=ロック中/0=ロックなし)。外部(Dashboard等)が
                                     //   InpEntryModeに依存せず参照できるよう独立公開(観測専用、buf60)。
...
void LoadTradeStateCacheAndOpenHandle()
{
   ...
   if(InpResetTradeStateLog && FileIsExist(g_tsLogPath))
   {
      ...
      Print("[indicator_14] InpResetTradeStateLog=true のため取引状態ログを削除しました: ", g_tsLogPath);
   }

   g_tsFileHandle = FileOpen(g_tsLogPath, FILE_READ|FILE_WRITE|FILE_CSV|FILE_ANSI, ',');
   if(g_tsFileHandle == INVALID_HANDLE)
   {
      Print("[indicator_14] 取引状態ログを開けません err=", GetLastError(), " path=", g_tsLogPath);
      return;
   }
   ...
   Print("[indicator_14] 取引状態ログ読込完了: ", g_tsCacheCount, "件 path=", g_tsLogPath);
}
...
int OnInit()
{
   ...
   SetIndexBuffer(60, BufReentryLock,            INDICATOR_CALCULATIONS); // ★2026-07-14: 再エントリーロック状態(buf60,観測専用)
   ...
   for(int i=need; i<rates_total; i++)
   {
      ...
      if(pos==0 && !justExited)
      {
         ...
         else
         {
            ...
            BufReentryLock[i] = (segHadEntry && d==trendDir) ? 1.0 : 0.0;   // ★2026-07-14追加: allowの状態に関わらず毎足記録(観測専用)
