//+------------------------------------------------------------------+
//|                              DokaKotsu_indicator_6.mq5            |
//|   バージョン : Ver6.0     修正日 : 2026-06-15 (JST)         |
//|                                                                  |
//|  ■■ 最重要・絶対ルール(EAと共通) ■■                            |
...
//=== バージョン情報(最新版か確認用) ==============================
#define DK_VERSION   "Ver6.0"
#define DK_BUILD     "2026-06-15 Ver6.0 (方向MA=KAMA25/色ヒステリシス/表示平滑)"
...
input ENUM_WMATYPE InpWmaType   = WT_KAMA; // InpWmaType: 方向判定MAの種類
input int          InpWmaPeriod = 20;      // InpWmaPeriod: 方向判定MAの期間
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "DokaKotsu_indicator_6");
   ...
   if(InpShowInfoLabel)
   {
      ...
      ObjectSetString (0, vname, OBJPROP_TEXT,
         StringFormat("DokaKotsu indicator_6  %s  (build %s)", DK_VERSION, DK_BUILD));
                ...
                const int &spread[])
{
   ...
   for(int i=need; i<rates_total; i++)
   {
      ...
      prevDirRun = wmaDir;
      if(wmaDir==1)      { BufWmaUp[i]=wmaShow[i];   BufWmaUp[i-1]=wmaShow[i-1]; }
      else if(wmaDir==-1){ BufWmaDown[i]=wmaShow[i]; BufWmaDown[i-1]=wmaShow[i-1]; }
