...
input int  InpMsgFreshSec  = 180;    // ★2026-07-15追加: 直近決済からこの秒数以内ならGET/LOSEを表示(一過性の演出)
...
input double InpAdxVolLow    = 1.6;   // これ未満=ボラ無し(グレー・動かず)
input double InpAdxVolMid    = 2.0;   // これ未満=イエロー
input double InpAdxVolHigh   = 3.5;   // これ未満=ライム。これ以上=レッド
...
void CreatePanel()
  {
   ...
   //   4段階: <1.6=グレー(動かず)/<2.0=イエロー/<3.5=ライム/>=3.5=レッド)
   ...
   if(InpUseRealLogic)
     {
      ...
      //   InpMsgFreshSec以内の決済だけを「今起きたこと」として一過性表示する(古い決済は出さない)。
      if(!showMsgIn)
        {
         ...
         if(GlobalVariableCheck(gvTime))
           {
            ...
            if(closeTime>0 && (TimeCurrent()-closeTime) <= InpMsgFreshSec)
              {
   ...
   CreateRectLabel("accbox",x+pad,curY,innerW,accH,COL_BG,COL_BORDER,1);
   {
      ...
      CreateLabelText("acclbl",x+pad+8,curY+accH/2-6,"▲ 本日の経済指標  "+dateStr,COL_GRAY2,6);
   }
   curY+=accH+(int)(12*sc);
