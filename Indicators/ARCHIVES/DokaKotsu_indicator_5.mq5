//+------------------------------------------------------------------+
//|                              DokaKotsu_indicator_5.mq5            |
//|   バージョン : Ver5.2     修正日 : 2026-06-15 (JST)         |
//|                                                                  |
//|  ■■ 最重要・絶対ルール(EAと共通) ■■                            |
...
//=== バージョン情報(最新版か確認用) ==============================
#define DK_VERSION   "Ver5.2"
#define DK_BUILD     "2026-06-15 Ver5.2 (色ヒステリシス追加=途切れ防止)"
...
input string InpVersionInfo = "V5.0 / 2026-06-11 背景色統合・HMA40既定"; // ★バージョン(最新確認用・変更不要)
input ENUM_WMATYPE InpWmaType   = WT_HMA; // ★方向判定のMA(種類)
input int          InpWmaPeriod = 40; // 方向判定のMAの期間
...
input double InpWmaStickyMult = 0.5;  // InpWmaStickyMult: 色の粘り(消灯=点灯×本値。小=途切れにくい)
input int    InpColorConfirmBars = 1; // ⑥色の確認本数(1=1本目の色で即エントリー=1本早い。レンジ除けはグレー基準の再エントリーが担うため1で運用。2,3で単発除去も可)
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "DokaKotsu_indicator_5");
   ...
   if(InpShowInfoLabel)
   {
      ...
      ObjectSetString (0, vname, OBJPROP_TEXT,
         StringFormat("DokaKotsu indicator_5  %s  (build %s)", DK_VERSION, DK_BUILD));
                ...
                const int &spread[])
{
   ...
   for(int i=need; i<rates_total; i++)
   {
      ...
      {
         color bgc = (wmaDir==1) ? InpColorBull :
                     (wmaDir==-1)? InpColorBear : InpColorRange;
