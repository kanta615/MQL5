...
input string InpStateLogDir     = "DokaKotsu\\state_log";     // 状態ログの保存フォルダ(MQL5\Files配下)
...
void AppendCachedState(const datetime t, const int state)
{
   ...
   int n = g_stateCacheCount;
   ArrayResize(g_stateCacheTime,  n+1);
   ArrayResize(g_stateCacheState, n+1);
   g_stateCacheTime[n]  = t;
   g_stateCacheState[n] = state;
   g_stateCacheCount = n+1;
}
...
void LoadStateCacheAndOpenHandle()
{
   ...
   FolderCreate(InpStateLogDir); // 既に存在してもエラーにはならない(戻り値は無視でよい)
   ...
   if(g_stateFileHandle == INVALID_HANDLE)
   {
      Print("[indicator_13] 状態ログを開けません err=", GetLastError(), " path=", g_stateLogPath);
      return;
   }
   ...
   Print("[indicator_13] 状態ログ読込完了: ", g_stateCacheCount, "件 path=", g_stateLogPath);
}
...
int OnInit()
{
   ...
   if(InpUseExternalSpikeForState)
   {
      ...
      if(hSpikeFilter == INVALID_HANDLE)
         Print("[indicator_13] ", InpSpikeFilterName, " のハンドル作成失敗。マーケットステイトのSPは常にfalse扱いになります。");
   }
                ...
                const int &spread[])
{
   ...
   if(InpUseExternalSpikeForState && hSpikeFilter != INVALID_HANDLE)
      extSpikeOk = (CopyBuffer(hSpikeFilter, 0, 0, rates_total, extSpikePass) > 0);
   if(extSpikeOk) ArraySetAsSeries(extSpikePass, false);
   else { ArrayResize(extSpikePass, rates_total); ArrayInitialize(extSpikePass, 0.0); }
   ...
   for(int i=need; i<rates_total; i++)
   {
      ...
      int marketState = stateSpikeActive ? 3 : (regimeSqueeze ? 1 : 2); // 1=SQ/2=TR/3=SP(最優先)
   ...
   if(doState)
   {
      ...
      for(int si = scanFrom; si < rates_total; si++)
      {
         ...
         color  col   = (curSt==1) ? clrGray : (curSt==3) ? clrLime : (color)C'34,116,128';
