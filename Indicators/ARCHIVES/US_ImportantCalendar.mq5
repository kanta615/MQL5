//+------------------------------------------------------------------+
//|                                        US_ImportantCalendar.mq5  |
//|                     米国重要経済指標カレンダー表示インジケーター          |
//|        対象: FOMC / 雇用統計 / CPI / GDP / PCE / 重要発言 のみ          |
...
string ClassifyEvent(const string rawName)
  {
   ...
   if(isSpeech)
     {
      if(StringFind(n, "POWELL")    >= 0) return "重要発言(パウエル)";
      if(StringFind(n, "FED CHAIR") >= 0) return "重要発言(議長)";
...
void DrawPanel(const datetime &evTimes[], const string &evLabels[], int total,
               int shift, datetime jstDayStart)
  {
   int y     = InpCornerY;
