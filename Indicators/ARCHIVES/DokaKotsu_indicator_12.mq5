...
input bool   InpM15ApplyToSell = false;                     // 2026-07-06追加(実験): false=SELL方向はM15フィルター対象外(下降時のM15平滑ラグで進み過ぎるのを回避)。BUYは常時適用
...
input bool   InpUseZzFilter     = false;                    // ZigZag(ATR型)の残存強度%フィルターを使うか。既定OFF=検証してからON推奨
