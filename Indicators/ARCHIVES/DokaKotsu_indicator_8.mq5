//+------------------------------------------------------------------+
//|                              DokaKotsu_indicator_8.mq5            |
//|   バージョン : Ver8.3(_8)  修正日 : 2026-06-24 (長期足MTF追加)|
//|                                                                  |
//|  ■■ 最重要・絶対ルール(EAと共通) ■■                            |
...
input string InpVersionInfo = "Ver8.0(_8) / 2026-06-20 方向MA=WMA34・案Cハイブリッド決済"; // ★バージョン(最新確認用・変更不要)
...
input int    InpM15Period    = 20;    // ★Ver8: 15分足MAの期間(WMA20)
...
input int          InpLongPeriod    = 360;     // ★長期足の期間(360)
input int          InpLongSlopeBars = 5;       // ★長期足の傾きを測る本数
input double       InpLongSlopeTh   = 0.10;    // ★長期足のグレー閾値(これ未満=ほぼ水平=グレー=待機)
...
input int            InpHaPostPeriod = 1;        // ★Ver7/2026-06-22: 決済用平均足:後平滑化の期間(5→1で反転を早め決済を1〜2本前倒し。前平滑5は維持)
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "DokaKotsu_indicator_8"); // ファイル名と一致させ括弧表示を解消
   ...
   if(InpShowInfoLabel)
   {
      ...
      ObjectSetString (0, vname, OBJPROP_TEXT,
         StringFormat("DokaKotsu indicator_8  %s  (build %s)", DK_VERSION, DK_BUILD));
                ...
                const int &spread[])
{
   ...
   for(int j=0; j<need && j<rates_total; j++)
   {
      ...
      BufReason[j]=0.0; BufM15State[j]=0.0;
   }
   ...
   for(int i=need; i<rates_total; i++)
   {
      ...
      // ★Ver8.3: SMA20_CENTERプロットを「長期足」(M5・既定KAMA360)に転用。
      //   表示色(5段階)と方向(longDir)を同じ閾値で出す=WYSIWYG。
      BufSma20[i]=longLine[i];
      int longDir = 0;   // 長期足の方向(0=グレー/1=上昇/-1=下降)。3本一致ゲートで使用。
      if(i-InpLongSlopeBars>=0 && atr[i]>0)
      {
         double lsl = (longLine[i]-longLine[i-InpLongSlopeBars])/atr[i];
         int sc;
         if(lsl >=  InpLongSlopeTh*1.5)      sc=0; // 強い上昇 アクア
         else if(lsl >=  InpLongSlopeTh*0.5) sc=1; // 上昇     水色
         else if(lsl <= -InpLongSlopeTh*1.5) sc=4; // 強い下降 マゼンタ
         else if(lsl <= -InpLongSlopeTh*0.5) sc=3; // 下降     薄マゼンタ
         else                                sc=2; // レンジ   グレー
         BufSma20Col[i]=sc;
         // 表示色と一致した3値方向(色つき=方向あり / グレー=待機)
         longDir = (sc==0 || sc==1) ? 1 : ((sc==3 || sc==4) ? -1 : 0);
      }
