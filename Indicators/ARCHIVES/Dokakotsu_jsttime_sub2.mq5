...
input color  InpAxisColor  = clrBlack;   // 文字色
input int    InpFontSize   = 8;          // 文字サイズ
input color  InpBgColor    = clrDimGray; // 背景色(clrNONEで背景を描画しない)
                ...
                const int &spread[])
{
   ...
   int off = IsSummerTime() ? 6 : 7;      // JST = server + off
   ...
   for(int i = begin; i < rates_total; i++)
   {
      ...
      if(!isNewDay && jt.min != 0) continue;   // 通常は毎正時(分=0)だけ
      ...
      if(isNewDay)
         ...
         txt = StringFormat("%d年%d月%d日(%s) %d:%02d",
                             jt.year, jt.mon, jt.day, wd[jt.day_of_week],
                             jt.hour, jt.min);
      else
         txt = StringFormat("%d時", jt.hour);
