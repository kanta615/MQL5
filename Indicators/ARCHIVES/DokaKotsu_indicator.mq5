...
//=== セッション背景(JST)パラメータ ==============================
input bool  InpShowSession  = true;   // 日本時間セッションの背景色を表示
input bool  InpAutoDST      = true;   // 夏/冬を自動判定するか(falseなら手動)
input bool  InpManualSummer = true;   // 手動時の夏時間ON/OFF(InpAutoDST=false時)
...
input bool  InpShowLabel    = true;   // 現在セッション名とJST時刻ラベル
input bool  InpShowJSTAxis  = true;   // チャート下部にJST毎正時ラベル
input color InpAxisColor    = clrWhite;// JST目盛りの文字色

string OBJ_PREFIX = "DKsess_";
string LBL_NAME   = "DKsess_label";
string AXIS_PREFIX= "DKjst_";
...
//=== セッション描画(OnCalculate末尾から呼ぶ) ====================
void DrawSessions(const int rates_total, const datetime &time[])
{
   if(!InpShowSession) return;
   int maxBars = 500;
   ...
   if(InpShowJSTAxis)
   {
      ObjectsDeleteAll(0, AXIS_PREFIX);
      int lbl=0;
      datetime tFrom = time[begin];
      datetime tTo   = time[rates_total-1];
      int off = IsSummerTime() ? 6 : 7;
      datetime hh = tFrom - (tFrom % 3600);
      for(datetime st=hh; st<=tTo; st+=3600)
      {
         datetime jst = st + off*3600;
         MqlDateTime jt; TimeToStruct(jst, jt);
         if(jt.min != 0) continue;
         string nm = AXIS_PREFIX + IntegerToString(lbl++);
         if(ObjectFind(0, nm) < 0)
            ObjectCreate(0, nm, OBJ_TEXT, 0, st, 0);
         ObjectSetInteger(0, nm, OBJPROP_TIME, 0, st);
         double pmin = ChartGetDouble(0, CHART_PRICE_MIN);
         ObjectSetDouble (0, nm, OBJPROP_PRICE, 0, pmin);
         ObjectSetString (0, nm, OBJPROP_TEXT, StringFormat("%d時", jt.hour));
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "DokaKotsu indicator");
                ...
                const int &spread[])
{
   ...
   DrawSessions(rates_total, time);
   return(rates_total);
}

//+------------------------------------------------------------------+
//| 終了時: セッション描画オブジェクトを削除                          |
//+------------------------------------------------------------------+
