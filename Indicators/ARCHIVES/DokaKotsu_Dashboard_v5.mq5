...
//|      決まるため、本体インジ/MarkerがSQ/TR/SP等のOBJ_TEXTを新しい   |
//|      足で作るたびにパネルの手前へ出てしまっていた。対策として      |
//|      CHART_EVENT_OBJECT_CREATEを有効化し、他インジがオブジェクトを |
//|      作ったのを検知した次のタイマー(最大1秒後)にパネル全オブジェ   |
//|      クトを削除→再作成して「最新=最前面」へ復帰する方式に変更     |
//|      (BringPanelToFront新設)。自分のPFX/RB_OBJは検知から除外して   |
//|      無限ループを防止。高頻度でオブジェクトを作り直すインジが      |
//|      同居しても作り直しが連発しないよう、復帰は最短5秒間隔に制限。 |
//|      ZORDER引き上げ(過去2回の対策)は残すが、    |
//|      これはクリック優先度用であり透け対策の本体は本方式である。    |
//|    ②(黒点滅の根治)毎秒のCreatePanel()が全オブジェクトへ           |
//|      ObjectSet〜を発行し、ティック起因の再描画がパスの途中に       |
//|      割り込むと中間状態(背景だけ等)が一瞬映る=下部ほど点滅して    |
//|      見えた。対策として全生成ヘルパー(CreateRectLabel/LabelText/   |
//|      ButtonObj)にダーティチェックを実装: 前回設定値の署名を        |
//|      キャッシュし、変化が無ければObjectSetを一切発行しない。       |
//|      通常パスの変更は点滅ラベル等の数個だけになり中間状態が        |
//|      ほぼ消える。あわせてChartRedrawも「実際に変更があった時       |
//|      だけ」呼ぶ(g_dirtyCount)。                                    |
//|    ③(負荷軽減)経済指標ファイル(CAL_FILE)を毎秒読んでいたのを      |
//|      60秒キャッシュに変更(内容は日次でしか変わらないため)。        |
//|      レインボーバーCanvasも進捗値が変わった時だけ再描画。          |
//|    ④(絶対ルール④準拠)v3はBOMなし+LF改行になっていたため、        |
//|      v4でUTF-8 BOM+CRLFに修正。                                   |
//|                                                                    |
...
//--- ★2026-07-21追加: レイヤー透け対策(前面復帰)+黒点滅対策(ダーティチェック)用の状態
bool   g_needBringFront = false;   // 他インジがオブジェクトを作った=パネルより手前に出た可能性(次のタイマーで前面復帰)
uint   g_lastFrontMs    = 0;       // 前回の前面復帰時刻(ms)。連続作成するインジがいても作り直し連発しないよう最短5秒間隔に制限
int    g_dirtyCount     = 0;       // このパスで実際にObjectSetを発行した数(0ならChartRedraw不要)
int    g_lastPanelH     = -1;      // 背景高さの前回値(変わった時だけOBJPROP_YSIZEを設定)
double g_lastRbProgress = -1.0;    // レインボーバーの前回進捗(変わった時だけCanvas再描画)
//--- オブジェクトごとの「前回設定値の署名」キャッシュ(線形検索。パネルは60個程度なので十分軽い)
string g_sigNames[];
string g_sigValues[];

//--- 署名キャッシュ: 前回と同じ署名なら-1(スキップ可)、違えば格納位置を返して更新する
bool SigUnchanged(const string full, const string sig)
  {
   int n = ArraySize(g_sigNames);
   for(int i=0; i<n; i++)
     {
      if(g_sigNames[i]==full)
        {
         if(g_sigValues[i]==sig) return true;   // 前回と完全一致=何も発行しなくてよい
         g_sigValues[i]=sig;                    // 変化あり=署名を更新して発行してもらう
         return false;
        }
     }
   ArrayResize(g_sigNames,  n+1);
   ArrayResize(g_sigValues, n+1);
   g_sigNames[n]=full;
   g_sigValues[n]=sig;
   return false;
  }

//--- 署名キャッシュ全消去(前面復帰でオブジェクトを作り直す時に必ず呼ぶ)
void SigCacheClear()
  {
   ArrayResize(g_sigNames,  0);
   ArrayResize(g_sigValues, 0);
  }
...
//+------------------------------------------------------------------+
//| ★2026-07-21追加: 経済指標ファイル(CAL_FILE)の60秒キャッシュ。      |
//|   従来は毎秒2回ファイルを開いて読んでいた(負荷+黒点滅の一因)。     |
//|   内容は日次でしか変わらないため60秒ごとの再読込で十分。           |
//|   日付跨ぎも最大60秒遅れで自動的に追従する。                       |
//+------------------------------------------------------------------+
bool   g_calCacheInit  = false;
uint   g_calCacheMs    = 0;
int    g_calCount      = 0;
string g_calLabels[];
string g_calHhmm[];
bool   g_calJpFound    = false;
string g_calJpName     = "";
string g_calJpHhmm     = "";

void RefreshCalendarCacheIfStale()
  {
   uint nowMs = GetTickCount();
   if(g_calCacheInit && (nowMs - g_calCacheMs) < 60000) return;   // uint減算なのでGetTickCountの49日ラップも安全
   g_calCacheInit = true;
   g_calCacheMs   = nowMs;
   g_calCount   = ReadTodayEconEvents(g_calLabels, g_calHhmm);
   g_calJpFound = ReadTodayJpHoliday(g_calJpName, g_calJpHhmm);
  }
...
void CreateRectLabel(string name,int x,int y,int w,int h,color bg,color border,int zorder=1)
  {
   ...
   // ★2026-07-21追加: ダーティチェック。前回と同じ内容でオブジェクトも実在するなら
   //   ObjectSetを一切発行しない(黒点滅対策の本体。中間状態の露出を最小化する)
   string sig=StringFormat("R|%d|%d|%d|%d|%d|%d|%d",x,y,w,h,(int)bg,(int)border,zorder);
   if(SigUnchanged(full,sig) && ObjectFind(0,full)>=0) return;
   g_dirtyCount++;
...
void CreateLabelText(string name,int x,int y,string text,color clr,int fontsize=8,string font="Arial",ENUM_ANCHOR_POINT anchor=ANCHOR_LEFT_UPPER)
  {
   string full=PFX+name;
   // ★2026-07-21追加: ダーティチェック(CreateRectLabelと同じ趣旨)
   string sig=StringFormat("L|%d|%d|%s|%d|%d|%s|%d",x,y,text,(int)clr,fontsize,font,(int)anchor);
   if(SigUnchanged(full,sig) && ObjectFind(0,full)>=0) return;
   g_dirtyCount++;
...
void CreateButtonObj(string name,int x,int y,int w,int h,string text,color bg,color txt,color border,int fontsize=7)
  {
   string full=PFX+name;
   // ★2026-07-21追加: ダーティチェック(CreateRectLabelと同じ趣旨)
   string sig=StringFormat("B|%d|%d|%d|%d|%s|%d|%d|%d|%d",x,y,w,h,text,(int)bg,(int)txt,(int)border,fontsize);
   if(SigUnchanged(full,sig) && ObjectFind(0,full)>=0) return;
   g_dirtyCount++;
...
void CreatePanel()
  {
   g_dirtyCount = 0;   // ★2026-07-21追加: このパスで実際に変更した数を数える(0なら再描画不要)
   int x=InpPanelX;
   int y=InpPanelY;
   int w=InpPanelWidth;
   int pad=14;
   ...
   int rbarH=(int)(16*sc);
     {
         ...
         if(RbCanvas.CreateBitmapLabel(0,0,RB_OBJ,barX,curY,barW,barH,COLOR_FORMAT_ARGB_NORMALIZE))
           {
            ...
            g_lastRbProgress=-1.0;   // ★2026-07-21追加: 作り直したCanvasは必ず初回描画させる
           }
         else
      ...
      // ★2026-07-21変更: 進捗が変わった時だけ再描画(従来は毎秒フルCanvas再描画していた)
      if(g_rbCanvasReady && MathAbs(demoProgress-g_lastRbProgress) > 0.0001)
        {
         DrawRainbowBarCanvas(barW,barH,demoProgress);
         g_lastRbProgress = demoProgress;
         g_dirtyCount++;
        }
     }
   curY+=rbarH+(int)(4*sc);
   ...
   // ★2026-07-21変更: 毎秒のファイル読込→60秒キャッシュ経由に変更
   RefreshCalendarCacheIfStale();
   string evLabels[], evHhmm[];
   ArrayCopy(evLabels, g_calLabels);
   ArrayCopy(evHhmm,   g_calHhmm);
   int evCount = g_calCount;
   ...
   // 背景サイズを実際のコンテンツ高さに合わせて確定(★2026-07-21変更: 変わった時だけ設定)
   int panelH=curY-y+10;
   if(panelH != g_lastPanelH)
     {
   ...
   // ★2026-07-21変更: 実際に何か変更した時だけ再描画する(無駄な強制再描画を廃止)
   if(g_dirtyCount > 0) ChartRedraw(0);
  }
...
int OnInit()
  {
   ...
   // ★2026-07-21追加: 他インジのオブジェクト作成イベントを受け取る(レイヤー透け対策の検知用)。
   //   OBJPROP_ZORDERは描画順を制御しない(クリック優先度のみ)ため、描画順=作成順で
   //   後から作られたSQ/TR/SP等に負けたら、検知して作り直すしかない。
...
//+------------------------------------------------------------------+
//| ★2026-07-21追加: パネル前面復帰。全パネルオブジェクトを削除して   |
//|   作り直す=作成順が最新になり、他インジのSQ/TR/SP等より手前に     |
//|   描かれるようになる。署名キャッシュも必ず消すこと(消さないと     |
//|   ダーティチェックが「変化なし」と判断して再作成をスキップし、    |
//|   パネルが消えたままになる)。                                     |
//+------------------------------------------------------------------+
void BringPanelToFront()
  {
   ObjectsDeleteAll(0, PFX);
   if(g_rbCanvasReady) { RbCanvas.Destroy(); g_rbCanvasReady=false; }
   ObjectDelete(0, RB_OBJ);
   SigCacheClear();
   g_lastPanelH     = -1;
   g_lastRbProgress = -1.0;
   CreatePanel();
  }
...
void OnTimer()
  {
   ...
   // ★2026-07-21追加: 前パスで他インジのオブジェクト作成を検知していたら、削除→再作成で
   //   最前面へ復帰する(最大1秒の遅れ)。通常時は従来通りCreatePanel(ダーティチェック付き)のみ。
   //   ※オブジェクトを高頻度で作り直すタイプのインジが同居していた場合に、前面復帰
   //     (削除→再作成)自体が毎秒発火して点滅要因になるのを防ぐため、復帰は最短5秒間隔。
   //     間隔未達の間はフラグを保持し、次に許可されたタイマーで必ず復帰する。
   if(g_needBringFront && (GetTickCount() - g_lastFrontMs) > 5000)
     {
      g_needBringFront = false;
      g_lastFrontMs = GetTickCount();
      BringPanelToFront();
      return;   // BringPanelToFront内でCreatePanel実行済み
     }
   CreatePanel();
  }
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
  {
   // ★2026-07-21追加: 他インジがオブジェクトを新規作成した=描画順(作成順)で当パネルより
   //   手前に出た可能性がある。自分自身(PFX接頭辞とレインボーバーCanvas)は除外しないと、
   //   前面復帰の再作成に自分で反応して無限ループになるため必ず除外する。
   //   ここではフラグを立てるだけにして、実際の復帰は次のOnTimer(最大1秒後)で1回だけ行う
   //   (新しい足では作成イベントが連続で来ることがあるため、その場で毎回作り直さない)。
   if(id == CHARTEVENT_OBJECT_CREATE)
     {
      if(StringFind(sparam, PFX) != 0 && sparam != RB_OBJ)
         g_needBringFront = true;
      return;
     }
