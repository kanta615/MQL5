//+------------------------------------------------------------------+
//|                              DokaKotsu_indicator_9.mq5            |
//|   バージョン : Ver9.0(_9)  修正日 : 2026-06-24 (8→9統一/案A他)|
//|                                                                  |
//|  ■■ 最重要・絶対ルール(EAと共通) ■■                            |
...
input string InpVersionInfo = "Ver9.0(_9) / 2026-06-25 長期線スムージング・平均足OHLC公開(前3/後5 SMMA)で表示=決済一致・buf15修正 / 2026-06-26 孤児ポジ救済=A案同期を双方向化(EA保有&インジ0なら実方向を採用→決済評価)"; // ★バージョン(最新確認用・変更不要)
...
//--- 波オシレーター（Wave_Sub から移管：数値を buf20-24 で公開） ---
input ENUM_WMATYPE InpWaveMaType = WT_KAMA; // 波:MAの種類
input int    InpWaveFast   = 14;            // 波:早い線の期間
input int    InpWaveSlow   = 34;            // 波:遅い線EMA期間
input int    InpWaveSignal = 10;            // 波:シグナル平滑
...
//--- ★Ver8.3: 長期足(M5上の遅いMA)。短期/中期/長期の3本パーフェクトオーダーの門番。
input bool         InpUseLongFilter = true;   // ★長期足一致フィルター(3本が同色に揃った時だけエントリー)
input bool   InpUseLongFirst   = true; // ★新: 長期が先頭に点灯(最後に点灯したのが長期ならエントリー禁止)
...
input int          InpLongPeriod    = 380;     // ★長期足の期間(380)
...
input double       InpLongGrayThresh  = 0.05;  // ★長期足のグレー閾値(ATR正規化slope。|傾き|<これ=ほぼ水平=グレー=待機)
input double       InpLongHystRatio   = 1.5;   // ★長期足: ヒステリシス(色に入る閾値=GrayThresh×これ。境界の青⇔グレーちらつき防止)
...
//--- ★出来高フィルター(2026-06-22)。薄商い(現在の出来高 < 直近平均×比率)は新規見送り。朝の低出来高レンジ除け。既定OFF(BT検証用)
...
input int            InpHaPrePeriod  = 3;        // ★2026-06-25: 決済用平均足:前平滑化の期間(SMOOTHED)。表示用HAインジと一致させる単一の真実
...
void Calc_ATRADAPT(double &out[], const double &close[], const double &atr[], const int rt)
{
   ArrayResize(out, rt);
   for(int i=0;i<rt;i++)
   {
...
void Calc_ATRTREND(double &out[], const double &close[], const double &atr[], const int rt)
{
   ArrayResize(out, rt);
   for(int i=0;i<rt;i++)
   {
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "DokaKotsu_indicator_9"); // ファイル名と一致させ括弧表示を解消
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
{
   int need = MathMax(MathMax(MathMax(InpEmaPeriod, InpSmaPeriod), InpWmaPeriod), InpLongPeriod) + 5;
   ...
   for(int j=0; j<need && j<rates_total; j++)
   {
      ...
      BufReason[j]=0.0; BufM15State[j]=0.0; BufLongState[j]=0.0;
   }
   ...
   //============ 波オシレーター（buf20-24 で公開：Wave_Sub が読む） ============
   {
      ...
      if(hWave!=INVALID_HANDLE)
      {
         ...
         if(CopyBuffer(hWave,0,0,rates_total,wvMA)<=0)
            for(int i=0;i<rates_total;i++) wvMA[i]=close[i];
      }
      else
      {
         switch(InpWaveMaType)
         {
            ...
            case WT_ATR_ADAPT: Calc_ATRADAPT(wvMA,close,atr,rates_total);                       break;
            case WT_ATR_TREND: Calc_ATRTREND(wvMA,close,atr,rates_total);                       break;
      ...
      for(int i=0;i<rates_total;i++)
      {
         ...
         double sg=(i==0)?wv:(wv*wpg+wPrevSig*(1.0-wpg)); wPrevSig=sg;
   ...
   for(int i=need; i<rates_total; i++)
   {
      ...
      if(pos==0 && !justExited)
      {
         ...
         else
         {
            ...
            if(InpUseLongFilter && longDir != d) { allow = false; BufReason[i]=22.0; } // 長期足不一致(グレー/逆)

            // ★新: 長期が先頭(最後に点灯したのが長期=後発ならNG)。長期runが他より短ければ後発。
            if(allow && InpUseLongFirst && !(longRun>=m15Run && longRun>=haRun && longRun>=colorRun))
            { allow = false; BufReason[i]=25.0; } // 長期が後発(=最後に点灯)→除外
            ...
            // ★新: Waveトリガー(最後の条件)。波がd方向(波線>シグナル=上/<シグナル=下)であること。
            if(allow && InpUseWaveTrigger)
            {
               bool waveOK = (d==1 && BufWaveVal[i] > BufWaveSig[i]) || (d==-1 && BufWaveVal[i] < BufWaveSig[i]);
               if(!waveOK) { allow = false; BufReason[i]=26.0; } // Waveが未反転(同方向でない)=待機
            }
