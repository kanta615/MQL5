...
//|          (2026-07-20 v5: 日本の祝日判定を追加。米国休日と同じ二段階   |
//|           フォールバック(①ブローカー配信MT5カレンダー(JP)を動的照会  |
//|           →②固定リスト(内閣府発表2026年・18日分))で判定する。       |
//|           このインジが日本祝日判定の唯一のソースとなり、判定結果      |
//|           (当日/停止中/開始・終了時刻)をGlobalVariable              |
//|           (DK_CAL_JPHOL_*_<magic>)へ書き出す。★絶対ルール1に従い、  |
//|           実際のエントリー停止ロジック(reason番号の割当等)は         |
//|           本体/Watchdogインジ側に実装すること。本ファイルは          |
//|           判定結果の提供とチャート表示・JSON出力のみを行う。         |
//|           停止時刻は「祝日当日14:00(JST)〜翌日0:00(JST)」の固定窓。  |
//|           (米国休日と異なりオセアニア再開待ちの計算は行わない=       |
//|           日付が変われば自動的に解除される単純な当日窓のため)。      |
//|          (2026-07-20(2回目) v6: DokaKotsu_Dashboard.mq5が日本祝日名  |
//|           を経済指標と同じ並び(白文字)で表示できるよう、監査用       |
//|           CAL_FILEに"HOLIDAY_JP|名前|停止時刻"行を追加書き出し。     |
//|           既存の"EVENT"行とは別キーワードのため、このインジ自身の    |
//|           パネル(LoadCalendarFromFile)には混ざらない=表示先ごとに    |
//|           行のキーワードを分けるだけで、判定ロジック自体は           |
//|           これまで通りこのファイルに一本化されたまま)。              |
//+------------------------------------------------------------------+
...
//--- 入力パラメーター（日本の祝日表示・停止判定用GV出力）★2026-07-20追加
input bool   InpShowJPHoliday      = true;         // 日本の祝日を表示・GV出力するか
input color  InpJPHolidayColor     = clrGold;       // 日本の祝日(予定)の表示色
input color  InpJPHolidayActiveColor = clrRed;      // 日本の祝日(停止中)の表示色
input int    InpJPHolStopHourJST   = 14;            // 日本の祝日 当日のエントリー停止時刻(JST・時)。本体/Watchdogが読むGVの計算にも使用(停止窓は当日この時刻〜翌日0:00 JST)
...
//--- ★2026-07-20追加: 日本の祝日判定結果を書き出すGlobalVariable名。
//    本体/Watchdogインジがこれを読んでエントリー停止ロジックに使う想定
//    (このインジは判定結果を渡すのみ。実際の停止判断・reason番号割当は
//    本体/Watchdog側の責務＝「ロジックはインジのみ」の絶対ルールに従う)。
#define GV_JPHOL_TODAY(m)  StringFormat("DK_CAL_JPHOL_TODAY_%d",  m)
#define GV_JPHOL_ACTIVE(m) StringFormat("DK_CAL_JPHOL_ACTIVE_%d", m)
#define GV_JPHOL_START(m)  StringFormat("DK_CAL_JPHOL_START_%d",  m)
#define GV_JPHOL_END(m)    StringFormat("DK_CAL_JPHOL_END_%d",    m)

//--- ★2026-07-20追加: 日本祝日判定結果(DetermineAndPublishJPHolidayが設定)。
//    パネル表示・JSON出力のためインジ内部で使い回すだけの保持用。
//    本体/Watchdog側は上記GlobalVariable経由で参照すること(直接この変数は見えない)。
string   g_jpHolName    = "";
bool     g_jpHolToday   = false;
bool     g_jpHolActive  = false;
datetime g_jpHolStart   = 0;
datetime g_jpHolEnd     = 0;
int      g_jpHolSrc     = 0;   // 0=なし 3=独自判定(動的照会) 4=固定リスト・要確認
...
//+------------------------------------------------------------------+
//| ★2026-07-20追加: 日本の祝日 固定リスト（フォールバック専用。ブローカー |
//|   のMT5カレンダーに"JP"の祝日データが配信されていない場合のみ使用）。 |
//|   内閣府発表の「国民の祝日」+ 振替休日 + 国民の休日を含む             |
//|   (2026年は合計18日。出典: 内閣府 令和8年国民の祝日・休日一覧)。     |
//|   毎年、内閣府の公表(前年の官報「暦要項」)を確認して更新すること。    |
//+------------------------------------------------------------------+
USHolidayEntry g_jpHolidays2026[] =
  {
   {"2026.01.01", "元日"},
   {"2026.01.12", "成人の日"},
   {"2026.02.11", "建国記念の日"},
...
//+------------------------------------------------------------------+
//| ★2026-07-20追加: 本日(JST)が日本の祝日かどうかを固定リストから判定  |
//|   (該当なしは"" を返す)                                            |
//+------------------------------------------------------------------+
string GetJPHolidayName(datetime jstDayStart)
  {
   string today = TimeToString(jstDayStart, TIME_DATE);
   int n = ArraySize(g_jpHolidays2026);
   for(int i = 0; i < n; i++)
      if(g_jpHolidays2026[i].dateStr == today)
         return g_jpHolidays2026[i].name;
   return "";
  }

//+------------------------------------------------------------------+
//| ★2026-07-20追加: 日本の祝日を判定し、結果をGlobalVariableへ書き出す。 |
//|   ①ブローカー配信のMT5カレンダー("JP")を動的照会                    |
//|   ②未配信/未検出なら固定リスト(g_jpHolidays2026)へフォールバック     |
//|   停止窓は「当日 InpJPHolStopHourJST時(JST) 〜 翌日0:00(JST)」の     |
//|   単純な当日窓(米国休日と違いオセアニア再開待ちの計算は不要なため)。  |
//+------------------------------------------------------------------+
void DetermineAndPublishJPHoliday(int shift, datetime jstDayStart, datetime serverFrom, datetime serverTo)
  {
   g_jpHolName = ""; g_jpHolToday = false; g_jpHolActive = false;
   g_jpHolStart = 0; g_jpHolEnd = 0; g_jpHolSrc = 0;

   if(InpShowJPHoliday)
     {
      //--- ①動的照会
      MqlCalendarValue jvalues[];
      int jcount = CalendarValueHistory(jvalues, serverFrom, serverTo, "JP");
      ...
      if(jcount > 0)
        {
         for(int i = 0; i < jcount; i++)
           {
            MqlCalendarEvent ev;
            if(!CalendarEventById(jvalues[i].event_id, ev)) continue;
            if(ev.type != CALENDAR_TYPE_HOLIDAY) continue;
            datetime t = jvalues[i].time;
            if(t < serverFrom || t >= serverTo) continue;
            g_jpHolName = ev.name;
            liveFound   = true;
            break;
           }
      ...
      if(liveFound)
        {
         g_jpHolToday = true;
         g_jpHolSrc   = 3;   // 独自判定(動的照会)
        }
      else
        {
         //--- ②固定リストへフォールバック
         string fallbackName = GetJPHolidayName(jstDayStart);
         if(fallbackName != "")
           {
            g_jpHolName  = fallbackName;
            g_jpHolToday = true;
            g_jpHolSrc   = 4;   // 固定リスト・要確認
           }
        }

      if(g_jpHolToday)
        {
         g_jpHolStart  = jstDayStart - shift + InpJPHolStopHourJST * 3600;   // 当日14:00(JST)などをサーバー時間へ換算
         g_jpHolEnd    = jstDayStart - shift + 24 * 3600;                    // 翌日0:00(JST)＝日付が変われば自動解除
         datetime now  = ServerNow();
         g_jpHolActive = (now >= g_jpHolStart && now < g_jpHolEnd);
        }
     }

   //--- ★本体/Watchdogインジ向けGlobalVariable出力。
   //    InpShowJPHoliday=falseの場合も含め、常に現在の判定結果(false/0)を書き込む。
   //    実際のエントリー停止判断・reason番号割当は本体/Watchdog側の責務。
   GlobalVariableSet(GV_JPHOL_TODAY(InpMagic),  g_jpHolToday  ? 1.0 : 0.0);
   GlobalVariableSet(GV_JPHOL_ACTIVE(InpMagic), g_jpHolActive ? 1.0 : 0.0);
   GlobalVariableSet(GV_JPHOL_START(InpMagic),  (double)g_jpHolStart);
   GlobalVariableSet(GV_JPHOL_END(InpMagic),    (double)g_jpHolEnd);
  }
...
void UpdateDisplay()
  {
   ...
   //--- ★2026-07-20追加: 日本の祝日判定 + GlobalVariable書き出し(本体/Watchdogが読む)。
   //    米国休日と違いEA側の判定は無いため、このインジ自身が唯一の判定ソースとなる。
   DetermineAndPublishJPHoliday(shift, jstDayStart, serverFrom, serverTo);
...
void SaveCalendarToFile(datetime jstDayStart, const datetime &evTimes[], const string &evLabels[],
                         ...
                         datetime holidayStart, datetime holidayEnd, int holidaySrc, int shift)
  {
   ...
   //--- ★2026-07-20(2回目)追加: 日本の祝日名をHOLIDAY_JP行として書き出す。
   //    既存の"EVENT"行とはキーワードを分けているため、LoadCalendarFromFile
   //    (このインジ自身のパネル用イベント一覧)には混ざらない。読み手
   //    (DokaKotsu_Dashboard.mq5)側でこのキーワードを個別に読む。
   if(g_jpHolToday)
     {
      string jline = StringFormat("HOLIDAY_JP|%s|%s", g_jpHolName, IntegerToString(InpJPHolStopHourJST) + ":00〜");
      FileWriteString(h, jline + "\r\n");
     }
   FileClose(h);
  }
...
void SaveCalendarToJson(datetime jstDayStart, const datetime &evTimes[], const string &evLabels[],
                        const int &evImportance[], int n, int shift)
  {
   ...
   //--- ★2026-07-20追加: 日本の祝日情報(monitor_4.py 経済指標タブ等での表示用)。
   //    該当なしの日は jp_holiday: null とする。
   string jpHolJson;
   if(g_jpHolToday)
     {
      string jpStartStr = TimeToString(g_jpHolStart + shift, TIME_DATE|TIME_MINUTES);
      StringReplace(jpStartStr, ".", "-");
      string jpEndStr = TimeToString(g_jpHolEnd + shift, TIME_DATE|TIME_MINUTES);
      StringReplace(jpEndStr, ".", "-");
      jpHolJson = "{\"name\":\"" + JsonEscape(g_jpHolName) + "\","
                + "\"active\":" + (g_jpHolActive ? "true" : "false") + ","
                + "\"stop_from\":\"" + jpStartStr + "\","
                + "\"resume_at\":\"" + jpEndStr + "\","
                + "\"source\":" + IntegerToString(g_jpHolSrc) + "}";
     }
   else
      jpHolJson = "null";
...
void DrawPanel(const datetime &evTimes[], const string &evLabels[], int total,
               ...
               const int holidaySrc)
  {
   ...
   //--- ★2026-07-20追加: 日本の祝日表示（該当日のみ）
   if(InpShowJPHoliday && g_jpHolToday)
     {
      string tagJP = (g_jpHolSrc == 3) ? "(動的判定)" : "(固定リスト・要確認)";

      string timeTxtJP;
      if(g_jpHolActive)
         timeTxtJP = "停止中(解除:" + TimeToString(g_jpHolEnd + shift, TIME_MINUTES) + ")";
      else
         timeTxtJP = TimeToString(g_jpHolStart + shift, TIME_MINUTES) + " 停止予定";

      color lineColorJP = g_jpHolActive ? InpJPHolidayActiveColor : InpJPHolidayColor;
      string holLineJP  = "★日本祝日" + tagJP + ": " + g_jpHolName + "  (" + timeTxtJP + ")";
      CreateLabel(PREFIX + "holidayjp", holLineJP, InpCornerX, y, lineColorJP, InpFontSize);
      y += lineH + 2;
     }
