...
//|    InpVolScoreLowPct(reason43の判定に使う閾値)の既定値を           |
//|    0%→10%に変更。DokaKotsu_Dashboard.mq5・DokaKotsu_VolScore_Sub.mq5|
//|    (表示専用)側の閾値も同日中に0→10/30→40へ変更されており、        |
//|    3ファイルとも同じグレー境界(10%)に揃えた。イエロー境界(表示側の |
//|    InpVolScoreMidPct)も30→40に変更されているが、本体インジの判定    |
//|    ロジックはグレー境界(Low)しか使わないため、Mid/Highの変更は     |
//|    本体側の判定には影響しない(表示専用ファイル側だけの変更)。      |
//|                                                                  |
//|  ■ 修正日 : 2026-07-21  修正内容(15→16へバージョンアップ・         |
//|    ボラティリティ(VolScore)を最終ゲートに追加)                    |
//|    DokaKotsu_Dashboard.mq5のComputeVolScore()と同じ式             |
//|      CurrentVol = EMA(High-Low, 5)                                |
//|      VolScore   = (CurrentVol - EMA(CurrentVol,20)) / EMA(それ,20)|
//|    を全バーぶん再帰計算し、新規buf61(BufVolScore,%表記)で公開。    |
//|    InpVolScoreLowPct(既定0%=ゼロライン。2026-07-21新設の           |
//|    DokaKotsu_VolScore_Sub.mq5と閾値の考え方を揃えた)未満を         |
//|    「グレー(ボラ不足)」と判定し、エントリー判定の最終チェックで    |
//|    ブロックする(reason43、InpUseVolScoreGateで on/off 可)。        |
//|    ロジックA/B/Cいずれで許可されていても、switch文でallowFinalが   |
//|    確定した直後にvolScoreGrayを共通で適用する設計のため、          |
//|    各ロジック個別の条件式そのものは変更していない(最終チェックを    |
//|    1箇所に集約=絶対ルール1「ロジックは一か所で管理」に従う)。      |
//|    ファイル名/#property version/IndicatorSetString/Printタグを    |
//|    15→16へ統一。                                                  |
//|                                                                  |
...
input bool   InpUseVolScoreGate  = true;   // VolScore最終ゲートを使う(グレーならreason43でブロック)
input double InpVolScoreLowPct   = 10.0;   // グレー境界(%)。2026-07-21(2回目): 0→10。DokaKotsu_Dashboard/VolScore_Subと揃えた
