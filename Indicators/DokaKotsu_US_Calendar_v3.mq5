...
//--- ★2026-07-07追加: 取得結果の出力先(MQL5\Files\配下。監査・デバッグ用の証跡)
...
void OnTimer()
  {
   ...
   if(jstDayStart != g_lastDrawnJstDay)   // 日付が変わった(または初回)時だけ再取得・再描画
      UpdateDisplay();
  }
...
void UpdateDisplay()
  {
   ...
   SaveCalendarToJson(jstDayStart, fEvTimes, fEvLabels, fEvImportance, fN, shift);
