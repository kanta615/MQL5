...
//|                                                                  |
//|  ■ このEAは何をするか(役割)                                     |
//|    DokaKotsu_indicator_10 が出すシグナルを「来たら速攻で実行」   |
...
//=== 入力 =========================================================
input string InpVersionInfo  = "v9.0 / EA_9 / 連敗ロット + 金曜停止 + イベント/米国休日/年末年始 + 停止理由GV + Discord / 2026-06-25 オセアニア解放07:00→08:58 / 2026-06-26 連敗カウントをポジション単位に修正(分割約定/✖再送の多重カウント解消)"; // ★バージョン(確認用)
...
input string InpIndicatorName = "DokaKotsu_indicator_10"; // 読み込むインジ名
...
int OnInit()
{
   ...
   Print("==== DokaKotsu_EA_9  build:v9.0-0624  起動(段階決済+再送ラッチ+MTF3本門番対応) ====");
   ...
   ObjectSetString (0, BUILD_LBL, OBJPROP_TEXT,       "DokaKotsu_EA build:v9.0-0624");
   ...
   if(InpDiscordOnStart)
      PostDiscord(StringFormat("【起動】DokaKotsu EA_9 %s  %s JST",
...
void OnTimer()
{
   ...
   if(InpDiscordHbMin > 0)
   {
      ...
      if(mktOpen && (g_lastHbDisc==0 || nowS - g_lastHbDisc >= (datetime)InpDiscordHbMin*60))
      {
         ...
         PostDiscord(StringFormat("【稼働】DokaKotsu EA_9  保有=%s  証拠金=%.0f  %s JST",
...
{
   ulong tk=0;
   if(CurrentPos(tk)==0 || tk==0){ g_peakPipMFE=0.0; g_mfeTicket=0; return; }
   if(tk != g_mfeTicket){ g_mfeTicket=tk; g_peakPipMFE=0.0; }  // 新ポジ=リセット
...
void RefreshUSHoliday()
{
   ...
   for(int i=0;i<cnt;i++)
   {
      ...
      datetime winEnd   = winStart + (datetime)((24 - InpUSHolStopHourJST) + InpResumeHourJST)*3600; // 翌 08:00(時間フィルターが08:58まで新規停止)
...
void OnTick()
{
   ...
   if(pos != 0)
   {
      ...
      {
         // ★段階決済(MAモード): buf25(背景方向)を自力で読み、反転/グレー継続で決済。
         //   インジの理由32待ちを廃止(再計算では保有中のgrayRunを積めないため)。EA変数なら確実に積める。
         ...
         // bg==-999(読取不可)はカウンタ据え置きで次ティック再評価
      }
   ...
   else if(pos == 0 && !inStop && !inEvent && !calBan && !usHol && !yearEnd && !inWeekendFlat && !inFriBan && !DK_IsDayPipStopped() && curBar != g_lastSignalBar)
   {
      ...
      {
         if(OpenTrade(ORDER_TYPE_BUY, useLot))  { g_lastSignalBar = curBar; g_lastExitMethod=0; g_stagedGrayRun=0; LogReason(curBar, "エントリーBUY"); }
      }
      else if(sSell && !sBuy)
      {
         if(OpenTrade(ORDER_TYPE_SELL, useLot)) { g_lastSignalBar = curBar; g_lastExitMethod=0; g_stagedGrayRun=0; LogReason(curBar, "エントリーSELL"); }
      }
   }
...
//+------------------------------------------------------------------+
//| 理由コード → 日本語(インジ DokaKotsu_indicator_10 と対応)       |
//+------------------------------------------------------------------+
...
void WriteReasonRow(datetime bt, int code, int pos, int cd, int tf, string eaNote)
{
   ...
   if(isNew)
      FileWrite(h, "time","dir","code","ha","long","m15","reason","ea_note","pos","cooldown","time_filter","order_err");
   FileWrite(h,
      TimeToString(btJst, TIME_DATE|TIME_MINUTES),
      ReasonDir(code), code, HaStateText(ReadHaState(0)), LongStateText(ReadLongState(0)), M15StateText(ReadM15State(0)), (g_lastExitMethod!=0 ? ReasonText(code)+" [直近決済:"+ExitMethodText(g_lastExitMethod)+"]" : ReasonText(code)), eaNote,
