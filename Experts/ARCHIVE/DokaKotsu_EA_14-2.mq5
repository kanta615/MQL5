...
void WriteReasonRow(datetime bt, int code, int pos, int cd, int tf, string eaNote)
{
   ...
   if(isNew)
      FileWrite(h, "time","dir","code","ha","long","m15","adx","zigzag","reason","ea_note","pos","cooldown","time_filter","order_err","shadow_wave","shadow_adx","shadow_zz","regime","regime_ratio","old_chain_reason");
   ...
   FileWrite(h,
      ...
      //   実際にエントリーした足でこの値が0以外なら「BB×KC限定にしたことで新たに拾えた機会」ということになる。
      (int)ReadBufRaw(59,0));
   FileClose(h);
