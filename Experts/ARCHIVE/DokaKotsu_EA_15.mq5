...
//|                              DokaKotsu_EA_15.mq5                  |
//|                                                                  |
//|  ■ 修正日: 2026-07-16  修正内容(小売売上高を停止対象に追加)      |
//|    EV_RETAIL新設。他5指標(FOMC/CPI/NFP/ISM/PPI)と全く同じ扱い。   |
//|    RefreshCalEvent()は指標ごとの個別ロジックを持たない設計なので、 |
//|    ClassifyEvent()に判定を追加するだけで、既存の共通ルール         |
//|    (InpEvStopHourJST=14時JST起点〜発表後InpEvAfterHours時間)が    |
//|    自動的に適用される。DK_StopReasonCode=14、evName="小売売上高    |
//|    発表前停止"。対になるDokaKotsu_US_Calendar側も追加済み。       |
//|                                                                  |
//|  ■ 修正日: 2026-07-15(2回目)  修正内容(14→15へバージョンアップ)  |
//|    エントリーリーズンの修正2点(WMA/M15タイミングずれ見送り、      |
//|    BB×KC未ブレイク見送り)に伴い、indicator/EA/ダッシュボードを    |
//|    まとめて14→15にリンク変更。参照先インジをDokaKotsu_indicator_15|
//|    へ更新。詳細はDK_BUILD参照。                                   |
//|                                                                  |
//|  ■ 修正日: 2026-07-15  修正内容(ダッシュボードの■/▶ボタンと連動) |
//|    DokaKotsu_Dashboard.mq5に新設した■(取引停止)/▶(復活)ボタンから |
//|    書き込まれるGlobalVariable(DK_DASH_TRADEPAUSE_<magic>)を読む   |
//|    IsDashboardPaused()を新設し、新規エントリーの統合判定           |
//|    (pos==0の大きなif条件)・停止理由テキスト・DK_StopReasonCode     |
//|    (新規コード13)に組み込んだ。保有中ポジションはこのボタンでは   |
//|    決済されない(あくまで新規エントリーの停止/再開のみ)。           |
//|                                                                  |
//|  ■ 修正日: 2026-07-15  修正内容(exit_groupに"RISK"分類を追加、     |
//|    ウォッチドッグ決済のexit_method衝突を修正)                     |
//|    田島さんの質問(「ウェーブ決済は何パターンあるか」)をきっかけに、|
//|    チャート判断(平均足/WMA/スパイク)以外に、EA自身のリスク管理に  |
//|    よる強制決済(緊急逆行/イベント/週末/ウォッチドッグ/SL)が別枠で  |
//|    5パターンあることが判明。このうちウォッチドッグ決済は           |
//|    g_lastExitMethod=33を使っており、indicator側のreason33          |
//|    (スパイク面積決済)と番号が衝突してexit_groupが誤って"SPIKE"に   |
//|    分類されるバグを発見。緊急逆行/イベント決済/週末強制決済も       |
//|    g_lastExitMethodを一切更新していなかった(直前の値が残ったまま)。|
//|    対策: 50=緊急逆行/51=イベント決済/52=週末強制/53=ウォッチドッグ  |
//|    の新番号を割り当て、ExitGroup()に"RISK"分類を追加。SL(ブローカー|
//|    側の逆指値直接約定)はOnTradeTransaction未実装のため現状          |
//|    result_snapshot自体が記録されない既知の課題として残す(54を予約)。|
//|                                                                  |
//|  ■ 修正日: 2026-07-15  修正内容(イベント新規禁止ロジック全面刷新) |
//|    従来「指標ごとに発表hb時間前から」(CPI/NFP/ISM/PPI=3h、        |
//|    FOMC=10h)+リーク時間帯(発表30分前は新規OK)+発表1分前の建値    |
//|    移動(スパイク保険)という仕組みだったが、対象5指標(FOMC/CPI/   |
//|    NFP/ISM/PPI)すべて共通で「発表当日のJST14時(InpEvStopHourJST) |
//|    から新規禁止、発表からInpEvAfterHours(既定1)時間後まで継続」   |
//|    というシンプルな方式に変更(米国休日停止と同じ考え方に統一)。   |
//|    発表が0時〜14時未満(FOMC等の深夜発表)の場合は前日14時が起点   |
//|    になるよう計算。InpFomcHoursBefore/InpCpiNfpHoursBefore/       |
//|    InpEvLeakMin/InpEvAfterMin/InpEvBeMinBefore・g_evNeedBE・      |
//|    ForceEventBreakeven()は削除。g_evStateは0/1のみ(リーク状態=2   |
//|    は廃止)。                                                     |
//|                                                                  |
//|  ■ 修正日: 2026-07-15  修正内容(【重大】entry/result JSONLの      |
//|    データ消失バグ修正)                                           |
//|    田島さんの環境で実際に確認: 05:40の記録がファイルに残った      |
//|    まま11:40に新規書込したところ、05:40分が消えて11:40分だけの    |
//|    1行になった(ファイルサイズが1行分のまま)。原因はMQL5の         |
//|    FileOpen(FILE_READ|FILE_WRITE)+FileSeek(SEEK_END)方式が、      |
//|    ドキュメント上は追記できるはずが実際には信頼できないという     |
//|    既知の問題(同様の報告がMQL5フォーラムに多数あり)。対策として、 |
//|    ファイルを都度開閉せずEA稼働中は開いたまま保持し               |
//|    (GetDailyFileHandle新設,g_entryFileHandle/g_resultFileHandle)、|
//|    日付が変わった時だけ閉じて次の日のファイルを新規に開く方式へ   |
//|    変更。OnDeinitでハンドルを確実にクローズ。                     |
//|                                                                  |
//|  ■ 修正日: 2026-07-15  修正内容(entry/result JSONLの文字コード   |
//|    バグ修正)                                                     |
//|    コードページ未指定のFileOpenだとシステムANSI(日本語Windowsは   |
//|    CP932)で書かれてしまい、Python側(json.load時UTF-8前提)で       |
//|    exit_why等の日本語項目が文字化けしていた(数値項目には影響なし)。|
//|    mt5_calendar_today.json(2026-07-08)で使ったCP_UTF8明示の       |
//|    書き方に統一して解消。実データ(result_20260715.jsonl)の生      |
//|    バイトでCP932混入を確認済み。                                 |
//|                                                                  |
//|  ■ 修正日: 2026-07-15  修正内容(決済理由の3分類=exit_group追加)  |
//|    田島さんとの会話で判明した誤解を訂正: reason34(ウェーブクロス  |
//|    救済)は独立した4つ目の決済理由ではなく、reason33(スパイク面積  |
//|    直接決済)が次の足で間に合わなかった時の保険であり、あくまで    |
//|    『スパイク決済』という1つの仕組みの一部。決済は本質的に平均足  |
//|    (30)/WMA(31,32)/スパイク(33,34)の3種類しかないという整理に     |
//|    基づき、ExitGroup()新設・result_snapshotにexit_group           |
//|    ("HA"/"WMA"/"SPIKE")を追加(記録専用・決済ロジックへの影響なし)。|
//|                                                                  |
//|  ■ 修正日: 2026-07-15  修正内容(PPIをイベント予防線の対象に追加) |
//|    EV_PPI新設。CPIと同じ扱い(発表前hbはInpCpiNfpHoursBeforeのまま |
//|    自動適用、DK_StopReasonCode=12、evName="PPI発表前停止")。       |
//|    対になるDokaKotsu_US_Calendar側もClassifyEvent/CategoryCodeに  |
//|    PPIを追加済み。                                                |
//|                                                                  |
//|  ■ 修正日: 2026-07-14  修正内容(Dashboard連携: 連勝数・直近決済)  |
//|    DK_UpdateGameStats()を新規追加。DK_LossStreak()と同じ考え方で  |
//|    直近ポジションを新しい順に集約し、①直近決済のpips・勝敗・     |
//|    決済時刻 ②本日の取引日分の連勝数 をGlobalVariableへ書き出す。 |
//|    (DK_EA_LASTCLOSE_PIPS_<magic> / DK_EA_LASTCLOSE_WIN_<magic> / |
//|     DK_EA_LASTCLOSE_TIME_<magic> / DK_EA_WINSTREAK_<magic>)      |
//|    DokaKotsu_Dashboard.mq5がこれを読み、GET/LOSEメッセージ箱と    |
//|    連勝/獲得pips表示を実データで動かせるようにするための追加。    |
//|    既存の連敗計算・発注ロジックへの影響なし(観測専用)。           |
//|                                                                  |
//|  ■ 修正日: 2026-07-12  修正内容(13→14へバージョンアップ)         |
//|    ①reason CSV・entry_snapshot(JSON)にBB/KCレジーム関連の値     |
//|      (regime=indicator buf53, regime_ratio=indicator buf58・     |
//|      新規追加)を出力するよう追加。判定ロジックへの影響なし・     |
//|      記録専用。                                                  |
//|    ②参照先インジをDokaKotsu_indicator_14に変更、13→14へ          |
//|      バージョンアップ(ファイル名/#property version/EA_VERSION)。 |
//|                                                                  |
//|  ■ 修正日: 2026-07-13  修正内容(バージョン番号は据え置き・実験運用)|
//|    パターンB対応: reason CSV・entry_snapshot(JSON)にold_chain_   |
//|    reason列(indicator buf59)を追加。indicator側がBB×KCのみで     |
//|    エントリー判定するモード(InpEntryModeBBKCOnly=true)の間、      |
//|    「もし従来の全フィルターのままだったらどう判定されたか」を     |
//|    確認できるようにした。EA自体の発注ロジック(buf7/8/9読取)は     |
//|    変更なし。決済ロジックへの影響なし。                           |
//|                                                                  |
//|  ■ 修正日: 2026-07-13  修正内容(ロジックC新規追加・indicator側)   |
//|    ロジックB(BB×KC/regimeSqueeze)は根本的にコントロールできないと|
//|    判断し中止(田島さんの判断)。indicator_14側にロジックC(WMA+   |
//|    M15同方向+確定足ガード)を新設し既定に変更。本EAはインジが     |
//|    出すBufBuy/BufSell/BufExitをそのまま実行するだけなので、       |
//|    本ファイル自体にコード変更は無い。記録として明記のみ。         |
//|                                                                  |
//|  ■ 修正日: 2026-07-13  修正内容(バージョン番号は据え置き)         |
//|    ①reason CSVにwave_color列を追加(indicator buf22=BufWaveReg)。 |
//|      チャート表示の波の色(オレンジ/ライム/グレー)をそのまま       |
//|      テキストで記録。reason26/27/28(波vsシグナルのクロス判定)     |
//|      とは別計算のため、区別できるよう追加した。記録専用。          |
//|    ②ロジックB(InpEntryModeBBKCOnly)に「15分足が同方向であること」|
...
input bool   InpUseCalEvent    = true; // ★MT5カレンダーでCPI/NFP/FOMC/ISM/PPI回避を使うか
// ★2026-07-15b修正: 「発表hb時間前から」(指標ごとにバラバラ)をやめ、対象5指標(FOMC/CPI/NFP/ISM/PPI)
//   すべて共通で「発表当日のJST14時から新規禁止」に統一(米国休日停止と同じ考え方)。
//   発表が0時〜14時未満(FOMC等の深夜発表)の場合は前日14時が起点になる。
...
input string InpIndicatorName = "DokaKotsu_indicator_15"; // 読み込むインジ名 ★2026-07-15(2回目): 14→15
...
int      g_exitKind          = 0;      // 1=✖ 2=緊急逆行 3=イベント 4=週末 5=ダッシュボード停止(2026-07-20追加)
...
//--- ★2026-07-15追加: entry/result JSONLを開きっぱなしにするための永続ハンドル(詳細はGetDailyFileHandle参照)
int      g_entryFileHandle  = INVALID_HANDLE;
datetime g_entryFileDate    = 0;
int      g_resultFileHandle = INVALID_HANDLE;
datetime g_resultFileDate   = 0;
...
int OnInit()
{
   ...
   DK_UpdateGameStats();             // ★2026-07-14追加: Dashboard連携用(連勝数・直近決済pips)
...
void OnDeinit(const int reason)
{
   ...
   // ★2026-07-15追加: 開きっぱなしにしているentry/result JSONLハンドルをEA終了時にちゃんと閉じる
   if(g_entryFileHandle  != INVALID_HANDLE) { FileFlush(g_entryFileHandle);  FileClose(g_entryFileHandle);  g_entryFileHandle  = INVALID_HANDLE; }
   if(g_resultFileHandle != INVALID_HANDLE) { FileFlush(g_resultFileHandle); FileClose(g_resultFileHandle); g_resultFileHandle = INVALID_HANDLE; }
}
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   ...
   if(DK_IsLossStopped())
   {
      ...
      DK_UpdateGameStats();             // ★2026-07-14追加
...
//+------------------------------------------------------------------+
//| ★2026-07-15追加: DokaKotsu_Dashboard.mq5の■/▶ボタンによる手動停止/復活。 |
//|   ダッシュボード側がGlobalVariableSet(DK_DASH_TRADEPAUSE_<magic>, 1)で |
//|   停止、0で復活を書き込む。GV未設定(ダッシュボード未起動)なら       |
//|   通常通り取引可(false)。                                          |
//+------------------------------------------------------------------+
bool IsDashboardPaused()
{
   string gv = StringFormat("DK_DASH_TRADEPAUSE_%d", InpMagic);
   return GlobalVariableCheck(gv) && (GlobalVariableGet(gv) != 0.0);
}
...
//+------------------------------------------------------------------+
//| ★MT5経済指標カレンダー: FOMC/CPI/NFP/ISM/PPI 回避                |
//|   ・カレンダーはサーバー時間で返る → サーバー時間で比較。        |
//|   ・状態: 0=影響なし / 1=新規禁止                                 |
//|   ・g_evKind に該当種別(理由表示用)。                            |
//|   ・毎ティックのカレンダー照会は重いので30秒キャッシュ。          |
//|   ★2026-07-15b: 「発表hb時間前から」(指標ごとにバラバラ)だった   |
//|     停止開始を、対象5指標すべて共通で「発表当日のJST14時から」に  |
//|     統一(米国休日停止と同じ考え方)。発表が0〜14時未満なら前日14時 |
//|     が起点になる。リーク時間帯・発表1分前の建値移動は廃止。       |
//+------------------------------------------------------------------+
enum EV_KIND { EV_NONE=0, EV_CPI=1, EV_NFP=2, EV_ISM=3, EV_FOMC=4, EV_PPI=5, EV_RETAIL=6 };   // ★2026-07-16: EV_RETAIL追加(他5指標と同じ扱い)

int      g_evState    = 0;        // 0 none / 1 ban
...
EV_KIND ClassifyEvent(const string name)
{
   ...
   if(StringFind(n,"consumer price")>=0 || StringFind(n,"cpi")>=0 ||
      ...
      return EV_CPI;
   // ★2026-07-15追加: PPI(生産者物価指数)。CPIと同じ扱い(停止開始は全指標共通のInpEvStopHourJST)
   if(StringFind(n,"ppi")>=0 || StringFind(n,"producer price")>=0 ||
      StringFind(name,"生産者物価")>=0)
      return EV_PPI;
   // ★2026-07-16追加: 小売売上高。他5指標と同じ扱い(停止開始は全指標共通のInpEvStopHourJST)
   if(StringFind(n,"retail sales")>=0 || StringFind(name,"小売売上高")>=0)
      return EV_RETAIL;
...
void RefreshCalEvent()
{
   ...
   g_evLastCalc = now;
   g_evState = 0; g_evKind = EV_NONE;
   if(!InpUseCalEvent) return;

   int off = IsSummerTime(now) ? InpJstOffSummer : InpJstOffWinter;   // ★米国休日と同じJST変換方式

   datetime from = now - 26*3600;   // ★停止開始が発表の前日14時までさかのぼる可能性があるため広めに取得
   datetime to   = now + 26*3600;
   MqlCalendarValue values[];
   int cnt = CalendarValueHistory(values, from, to, "US");
   if(cnt <= 0) return;

   bool banFound=false;
   EV_KIND banKind=EV_NONE;
   for(int i=0;i<cnt;i++)
   {
      MqlCalendarEvent ev;
      ...
      // ★2026-07-15b: 発表当日のJST InpEvStopHourJST 時を起点にする。
      //   発表がその時刻より前(深夜〜午前)なら、前日の同時刻を起点にする。
      datetime evtJst = evt + off*3600;
      MqlDateTime evtJstSt; TimeToStruct(evtJst, evtJstSt);
      evtJstSt.hour = InpEvStopHourJST; evtJstSt.min = 0; evtJstSt.sec = 0;
      datetime deadStartJst = StructToTime(evtJstSt);
      if(deadStartJst > evtJst) deadStartJst -= 24*3600;   // 発表が起点時刻より前なら前日を使う
      datetime deadStart = deadStartJst - off*3600;        // サーバー時間へ戻す
...
//|  6=米国休日 7=日本休日(年末年始) 8=連敗 9=週末クローズ 10=日次損失 11=ISM(2026-07-06追加) 12=PPI(2026-07-15追加) 13=ダッシュボード手動停止(2026-07-15追加) 14=小売売上高(2026-07-16追加) |
//+------------------------------------------------------------------+
int DK_StopReasonCode(bool dayStop, bool lossStop, bool inWeekendFlat, bool inFriBan,
                      bool usHol, bool yearEnd, bool inStop, bool dashPause=false)
{
   if(dashPause)     return 13;   // ★2026-07-15追加: ダッシュボードの■ボタンによる手動停止(最優先表示)
   if(dayStop)       return 10;   // ★2026-06-22 日次損失上限
   ...
   if(g_evState==1)                       // イベント新規禁止中
   {
      ...
      if(g_evKind==EV_PPI)    return 12;    // ★2026-07-15追加
      if(g_evKind==EV_RETAIL) return 14;    // ★2026-07-16追加
      return 3;                            // CPI
   }
...
//+------------------------------------------------------------------+
//| ★2026-07-14追加: ダッシュボード連携用。直近決済(勝敗問わず)の      |
//|   pips・時刻と、連勝数(本日の取引日分)をGlobalVariableへ書き出す。 |
//|   DK_LossStreak()と同じ「ポジションID単位に集約→新しい順ソート」  |
//|   をこの関数専用に複製している(安全のため共有変数化はしていない)。|
//+------------------------------------------------------------------+
void DK_UpdateGameStats()
{
   long todayId = DK_TradeDayId(TimeCurrent());
   datetime from = TimeCurrent() - 60*60*24*60;
   if(!HistorySelect(from, TimeCurrent())) return;
   int total = HistoryDealsTotal();

   long     ids[]; double pls[]; datetime tms[];
   int n = 0;
   for(int i=0; i<total; i++)
   {
      ulong t = HistoryDealGetTicket(i);
      ...
      long   pid = (long)HistoryDealGetInteger(t, DEAL_POSITION_ID);
      double pl  = HistoryDealGetDouble(t, DEAL_PROFIT) + HistoryDealGetDouble(t, DEAL_SWAP) + HistoryDealGetDouble(t, DEAL_COMMISSION);
      datetime dt = (datetime)HistoryDealGetInteger(t, DEAL_TIME);

      int idx=-1;
      for(int j=0; j<n; j++) if(ids[j]==pid){ idx=j; break; }
      if(idx<0){ idx=n; n++; ArrayResize(ids,n); ArrayResize(pls,n); ArrayResize(tms,n); ids[idx]=pid; pls[idx]=0.0; tms[idx]=0; }
      pls[idx]+=pl;
      if(dt>tms[idx]) tms[idx]=dt;
   }
   if(n==0) return;
   ...
   for(int a=0; a<n-1; a++)
      for(int b=a+1; b<n; b++)
         if(tms[b] > tms[a])
         { datetime tt=tms[a]; tms[a]=tms[b]; tms[b]=tt; double pp=pls[a]; pls[a]=pls[b]; pls[b]=pp; long ii=ids[a]; ids[a]=ids[b]; ids[b]=ii; }

   // ── 直近決済(新しい1件)のpipsを算出(DK_DayPipと同じIN対応付け) ──
   double lastPips = 0.0;
   {
      long posid = ids[0];
      double outPrice=0.0; int outType=-1;
      for(int i=0;i<total;i++)
      {
         ulong t=HistoryDealGetTicket(i); if(t==0) continue;
         if((long)HistoryDealGetInteger(t,DEAL_POSITION_ID)!=posid) continue;
         ENUM_DEAL_ENTRY e=(ENUM_DEAL_ENTRY)HistoryDealGetInteger(t,DEAL_ENTRY);
         if(e==DEAL_ENTRY_OUT || e==DEAL_ENTRY_INOUT){ outPrice=HistoryDealGetDouble(t,DEAL_PRICE); }
      }
      double inPrice=0.0; int inType=-1;
      for(int k=0;k<total;k++)
      {
         ulong tk=HistoryDealGetTicket(k); if(tk==0) continue;
         ...
         inPrice=HistoryDealGetDouble(tk,DEAL_PRICE); inType=(int)HistoryDealGetInteger(tk,DEAL_TYPE); break;
      }
      if(inPrice>0.0 && PIP>0.0)
         lastPips = (inType==DEAL_TYPE_BUY) ? (outPrice-inPrice)/PIP : (inPrice-outPrice)/PIP;
   }

   // ── 連勝数(本日の取引日分のみ、DK_LossStreakと対称のロジック) ──
   int winStreak=0;
   for(int i=0;i<n;i++)
   {
      if(DK_TradeDayId(tms[i]) != todayId) break;
      if(pls[i] >= 0.0) winStreak++;
      else break;
   }

   GlobalVariableSet(StringFormat("DK_EA_LASTCLOSE_PIPS_%d", InpMagic), lastPips);
   GlobalVariableSet(StringFormat("DK_EA_LASTCLOSE_WIN_%d",  InpMagic), (pls[0]>=0.0)?1.0:0.0);
   GlobalVariableSet(StringFormat("DK_EA_LASTCLOSE_TIME_%d", InpMagic), (double)tms[0]);
   GlobalVariableSet(StringFormat("DK_EA_WINSTREAK_%d",      InpMagic), (double)winStreak);
}
...
bool CheckEmergencyStop(int pos)
{
   ...
   if(adversePips >= InpEmergencyPips)
   {
      ...
      g_lastExitMethod = 50;   // ★2026-07-15追加: exit_group="RISK"用(EA側リスク管理の強制決済)
...
void OnTick()
{
   ...
   if(curBar != s_lastStreakBar) { s_lastStreakBar = curBar; g_lossStreak = DK_LossStreak(); g_dayPip = DK_DayPip(); DK_UpdateGameStats(); UpdateResumeButton(); }

   bool inStop  = IsInStopTime();
   bool inEvent = IsInEventWindow();   // FOMC/NFP予防線の窓内か(手入力・既定OFF)
   bool dashPause = IsDashboardPaused();   // ★2026-07-15追加: ダッシュボードの■/▶ボタンによる手動停止

   // ★MT5カレンダー: FOMC/CPI/NFP/ISM/PPI と 米国休日 を更新
   RefreshCalEvent();
   RefreshUSHoliday();
   bool calBan   = (g_evState == 1);   // イベント新規禁止(JST14時起点〜発表後InpEvAfterHours時間)
   bool usHol    = g_usHol;            // 米国休日(JST18:00〜翌07:00)
   bool yearEnd  = IsYearEndStop();    // 年末年始(12/30〜1/3)

   // ★2026-07-15b: イベント直前の建値移動(スパイク保険)は廃止(田島さんの整理に合わせ、リーク時間帯と一緒に削除)。
   ...
   if(inEvent && InpEventFlatten && pos != 0)
   {
      ...
      g_lastExitMethod = 51;   // ★2026-07-15追加: exit_group="RISK"用
   ...
   if(inWeekendFlat && pos != 0)
   {
      ...
      g_lastExitMethod = 52;   // ★2026-07-15追加: exit_group="RISK"用
      LogReason(curBar, "週末強制決済");
      pos = CurrentPos(tk);
   }

   // ★2026-07-20追加: ダッシュボードの■(停止)ボタン=手動操作による強制停止。
   //   保有中に押された場合は新規禁止だけでなく、その場で保有も決済する
   //   (田島さん要望: 「保有している場合は決済してから停止。保有していなければ停止だけでよい」)。
   //   保有していなければIsDashboardPaused()による新規禁止判定(既存)のみ働く=このブロックは素通り。
   if(dashPause && pos != 0)
   {
      CommandExit(5, "ダッシュボード停止");   // ★1回失敗で諦めず再送(失敗①対策、他の強制決済と同じ仕組み)
      g_lastExitMethod = 55;   // ★2026-07-20追加: exit_group="RISK"用
      LogReason(curBar, "ダッシュボード停止決済");
      pos = CurrentPos(tk);
   }
   ...
   // ★ウォッチドッグの決済指示(GV)を最優先で受信 → 即決済+再送(EAとは独立した安全網)
   {
      ...
      if(req != 0.0)
      {
         if(pos != 0)
         {
            ...
            // ★2026-07-15修正: g_lastExitMethod=33はindicator側のreason33(スパイク面積決済)と衝突していた
            //   (ExitGroup(33)="SPIKE"になってしまうバグ)。ウォッチドッグ専用の53に変更して解消。
            //   ※CommandExitの第1引数(kind=33)はexit_kind用の別の番号体系で、こちらは変更していない。
            g_lastExitMethod = 53;
   ...
   else if(pos == 0 && !inStop && !inEvent && !calBan && !usHol && !yearEnd && !inWeekendFlat && !inFriBan && !DK_IsDayPipStopped() && !dashPause && curBar != g_lastSignalBar)
   {
   ...
   if(curBar != g_lastBarTime)
   {
      ...
      bool lossStop = DK_IsLossStopped();
      bool dayStop  = DK_IsDayPipStopped();
      string evName = (g_evKind==EV_FOMC)?"FOMC発表前停止":(g_evKind==EV_NFP)?"雇用統計発表前停止":(g_evKind==EV_ISM)?"ISM発表前停止":(g_evKind==EV_PPI)?"PPI発表前停止":(g_evKind==EV_RETAIL)?"小売売上高発表前停止":"CPI発表前停止";
      string note = (pos!=0) ? "保有中"
                  : (dashPause     ? "手動停止中(ダッシュボード)"
                  : (dayStop       ? "日次損失上限停止"
                  : (lossStop      ? "連敗停止"
                  : (yearEnd       ? "年末年始停止"
                  : (usHol         ? "米国休日停止"
      ...
      int rcode = DK_StopReasonCode(dayStop, lossStop, inWeekendFlat, inFriBan, usHol, yearEnd, inStop, dashPause);
      g_stopReason = rcode;
      GlobalVariableSet(StringFormat("DK_EA_STOPREASON_%d", InpMagic), (double)rcode);
      WriteReasonRow(curBar, code, pos, 0, (int)(inStop||inEvent||calBan||usHol||yearEnd||inFriBan||dashPause), note);
   }
}
...
//--- ★2026-07-13追加: チャート表示の波(オレンジ/ライム/グレー)そのものの状態文字。
//    buf22(BufWaveReg)=短期WMA34自身の傾きグレーライン判定。reason26/27/28の元(波vsシグナル)とは別物。
string WaveColorText(int s){ return (s==1) ? "波上昇(ライム)" : (s==-1) ? "波下降(オレンジ)" : "波グレー"; }
...
void WriteReasonRow(datetime bt, int code, int pos, int cd, int tf, string eaNote)
{
   ...
   if(isNew)
      FileWrite(h, "time","dir","code","ha","long","m15","adx","zigzag","reason","ea_note","pos","cooldown","time_filter","order_err","shadow_wave","shadow_adx","shadow_zz","regime","regime_ratio","old_chain_reason","wave_color");
   ...
   FileWrite(h,
      ...
      // ★2026-07-12追加: BB×KCレジーム(buf53=0/1・[SQ]/[TR]/[SP]の裏側で常にSQと出るとは限らないため、
      //   独立の列として毎回出力)とレジーム圧縮比率(buf58,indicator_14新規追加。1.0未満=圧縮/1.0以上=解放)。
      //   ①のご要望対応。判定には未使用の観測専用。
      (int)ReadBufRaw(53,0), StringFormat("%.3f", ReadBufRaw(58,0)),
      // ★2026-07-13追加(パターンB比較用): old_chain_reason(buf59)。indicator側でInpEntryModeBBKCOnly=trueの間、
      //   実際の発注はBB×KC(regime)+再エントリーロックのみで行われるが、この列には「もし従来の
      //   全フィルター(M15/Wave/ZigZag/ADX/長期MTF等)のままだったらどう判定されたか」が入る
      //   (0=旧ロジックでも許可していたはず/それ以外=旧ロジックのブロック理由コード)。
      (int)ReadBufRaw(59,0),
      // ★2026-07-13追加: チャート表示の波の色(buf22=BufWaveReg)をそのままテキストで記録。
      //   reason26/27/28(波vsシグナルのクロス)とは別計算。田島さんが実際に見ている色と一致させるための列。
...
//+------------------------------------------------------------------+
//| ★2026-07-15追加: entry/result JSONLの「開いて閉じるたびに既存内容が |
//|   消える」バグへの対策。MQL5のFileOpen(FILE_READ|FILE_WRITE)は     |
//|   ドキュメント上はFileSeek(SEEK_END)で追記できるはずだが、実際には  |
//|   同じ問題を訴えるフォーラム報告が多数あり、田島さんの環境でも      |
//|   実際に05:40の記録が11:40の書込で消える(ファイルサイズが1行分の   |
//|   まま)という形で再現した。対策として、ファイルを都度開閉せず      |
//|   EA稼働中は開いたまま保持し、日付が変わった時だけ閉じて次の日の    |
//|   ファイルを新規に開く方式に変更する。グローバル変数はOnInitより    |
//|   前(ファイル先頭側)で宣言済み(g_entryFileHandle等)。              |
//+------------------------------------------------------------------+
int GetDailyFileHandle(int &handleVar, datetime &dateVar, const string prefix)
{
   MqlDateTime dt; TimeToStruct(TimeCurrent(), dt);
   dt.hour = 0; dt.min = 0; dt.sec = 0;
   datetime today0 = StructToTime(dt);

   if(handleVar != INVALID_HANDLE && dateVar == today0)
      return handleVar;   // ★既に今日のファイルを開いたまま保持している→そのまま使う(再オープンしない)

   if(handleVar != INVALID_HANDLE)
   {
      FileFlush(handleVar);
      FileClose(handleVar);   // 日付が変わった→前日分のハンドルを閉じる
   }

   string fname = StringFormat("%s\\%s_%04d%02d%02d.jsonl", InpReasonDir, prefix, dt.year, dt.mon, dt.day);
   handleVar = FileOpen(fname, FILE_READ|FILE_WRITE|FILE_TXT|FILE_ANSI, 0, CP_UTF8);
   if(handleVar == INVALID_HANDLE)
   {
      Print("[EA] ", prefix, " JSON書込失敗 err=", GetLastError());
      dateVar = 0;
      return INVALID_HANDLE;
   }
   FileSeek(handleVar, 0, SEEK_END);   // ★このファイルを開いた直後、1回だけ末尾へ移動(以後はこのハンドルを使い回すので毎回のseekは不要)
   dateVar = today0;
   return handleVar;
}
...
void WriteEntrySnapshot(int dir)
{
   ...
   datetime now = TimeCurrent();
   // ★2026-07-15修正: 開閉のたびに既存内容が消えるバグ対策として、日付をまたぐまでハンドルを使い回す方式に変更
   int h = GetDailyFileHandle(g_entryFileHandle, g_entryFileDate, "entry");
   if(h == INVALID_HANDLE) return;   // ★エラーはGetDailyFileHandle内でPrint済み
   ...
   // ★2026-07-12追加: regime_ratio(buf58,indicator_14新規)。BB×KC圧縮比率の生値
   //   (1.0未満=圧縮/1.0以上=解放)。regime(0/1)だけでは分からない「どれだけ圧縮/解放されているか」の
   //   連続値を記録。判定には未使用の観測専用。
   // ★2026-07-13追加(パターンB比較用): old_chain_reason(buf59)。実際の発注判定には未使用、
   //   「もし従来の全フィルターのままだったらどう判定されたか」の参考値(0=旧ロジックも許可)。
   string judgment = StringFormat(
      ...
      "\"regime\":%d,\"regime_ratio\":%.3f,\"old_chain_reason\":%d,\"shadow_wave\":%d,\"shadow_adx\":%d,\"shadow_zz\":%d}",
      ...
      (int)ReadBufRaw(53,0), ReadBufRaw(58,0), (int)ReadBufRaw(59,0), (int)ReadBufRaw(54,0), (int)ReadBufRaw(55,0), (int)ReadBufRaw(56,0));
   ...
   FileFlush(h);   // ★2026-07-15修正: FileCloseしない(ハンドルを保持し続ける)。クラッシュ対策でFlushのみ実施
}
...
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| ★2026-07-15追加: 決済方式(exit_method 30-34)を3分類へ集約する。    |
//|   田島さんの整理により、決済ロジックは本質的に平均足/WMA/スパイクの |
//|   3種類しかなく、reason34(ウェーブクロス救済)は独立した4つ目の理由 |
//|   ではなく「スパイク決済(33)が次の足で間に合わなかった時の保険」    |
//|   という、あくまでスパイク決済の一部であることが判明したため、       |
//|   33/34を同じ"SPIKE"グループにまとめる。                            |
//|     30       = HA  (平均足反転)                                    |
//|     31,32    = WMA (MA転換・MAグレー)                              |
//|     33,34    = SPIKE(スパイク面積直接決済・間に合わなかった時の     |
//|                       ウェーブクロス救済。どちらもスパイク起因)      |
//|   ★2026-07-15b追加: チャート判断(平均足/WMA/スパイク)とは別に、    |
//|   EA自身のリスク管理による強制決済を"RISK"として分離する。          |
//|   従来はexit_methodが更新されないまま(直前の値が残る)だったため、   |
//|   ウォッチドッグ決済がg_lastExitMethod=33のままreason33(スパイク)と |
//|   衝突していた実バグを含め、5パターンに新しい番号を割り当てた。      |
//|     50 = 緊急逆行ストップ(InpEmergencyPips)                        |
//|     51 = イベント決済(経済指標発表前後)                            |
//|     52 = 週末強制決済                                              |
//|     53 = ウォッチドッグ決済(旧33から変更・衝突解消)                |
//|     55 = ダッシュボード停止ボタンによる強制決済(2026-07-20追加)    |
//|   ※SL(ブローカー側の逆指値直接約定)はOnTradeTransaction未実装のため|
//|     現状WriteResultSnapshot自体が呼ばれず記録できない(既知の課題)。 |
//+------------------------------------------------------------------+
string ExitGroup(int method)
{
   if(method==30)              return "HA";
   if(method==31 || method==32) return "WMA";
   if(method==33 || method==34) return "SPIKE";
   if(method>=50 && method<=55) return "RISK";
   return "OTHER";
}
...
void WriteResultSnapshot(double pips, int exitKind, const string why)
{
   ...
   // ★2026-07-15修正: 開閉のたびに既存内容が消えるバグ対策として、日付をまたぐまでハンドルを使い回す方式に変更
   int h = GetDailyFileHandle(g_resultFileHandle, g_resultFileDate, "result");
   if(h == INVALID_HANDLE) return;   // ★エラーはGetDailyFileHandle内でPrint済み
   ...
   string json = StringFormat(
      ...
      "\"exit_method\":%d,\"exit_group\":\"%s\",\"spike_area\":%.1f}",
      ...
      g_lastExitMethod, ExitGroup(g_lastExitMethod), ReadBufRaw(41, 0));
