...
void OnTick()
{
   ...
   if(pos != 0)
   {
      ...
      else if(sExit)
      {
         ...
         bool doExit;
         if(stagedMA) doExit = (rc != 30); // MAモード: 平均足(30)は無視。MA(31)/グレーアウト(32)で決済
         else         doExit = true;       // 通常(トリガー未満): どの理由でも決済(平均足を含め早出し)
         if(doExit)
         {
...
void WriteReasonRow(datetime bt, int code, int pos, int cd, int tf, string eaNote)
{
   ...
   FileWrite(h,
      ...
      ReasonDir(code), code, HaStateText(ReadHaState(0)), LongStateText(ReadLongState(0)), M15StateText(ReadM15State(0)), ReasonText(code), eaNote,
