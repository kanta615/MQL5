...
void UpdateMFE(int pos)
{
   ...
   if(tk != g_mfeTicket){ g_mfeTicket=tk; g_peakPipMFE=0.0; g_trailArmed=false; }  // 新ポジ=リセット
...
void WriteReasonRow(datetime bt, int code, int pos, int cd, int tf, string eaNote)
{
   ...
   FileWrite(h,
      ...
      ReasonDir(code), code, HaStateText(ReadHaState(0)), LongStateText(ReadLongState(0)), M15StateText(ReadM15State(0)), (g_lastExitMethod!=0 ? ReasonText(code)+" [直近決済:"+ExitMethodText(g_lastExitMethod)+"]" : ReasonText(code)) + StringFormat(" [ATR:%.1fpip]", AtrPips(1)), eaNote,
